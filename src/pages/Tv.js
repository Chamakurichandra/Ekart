import React from 'react';
import Header from "../Components/Header";
import Footer from "../Components/Footer";
import {Link} from "react-router-dom";
export default class Tv extends React.Component{
    constructor(){
        super();
        this.state={
             x:null
        }
    };
        changeState=()=>{

            this.setState({
            
                x:this.state.x+1
               
            })
            console.log("added")
        };

    render(){
        return(
            <div>
            <Header sub = {this.state.x} />
        <div className="same-all">
            <div className="row">
            <div className="col">
               <div>
               <img  className="images" src="https://i.postimg.cc/d3mMT62f/TV-1.jpg" width="80%" height="500px" alt=""/>
               </div>
               <div>
                   <button type="button" onClick={this.changeState} className="btn btn-warning text-white cart"><i className="fa fa-shopping-cart"></i>ADD TO CART</button>
                   <Link to="/CartDetails7">
                   <button type="button" className="btn btn-danger cart">BUY NOW</button>
                   </Link>
               </div>
            </div>
            <div className="col">
                <div className="product-details">
                <h5>Samsung 80cm (32 inch) HD Ready LED Smart TV  (UA32T4340AKXXL)</h5>
                <b className="rating">4.3<i className="fa fa-star"></i></b><span>rating</span>
                <h5> &#8377;28000/-<small className="strike"> &#8377;31000/-</small></h5>
                <h4>Available Offers</h4>
                <p>Bank Offers 5% Unlimited Cashback On Flipkart Axis Bank Credit Card <a href="#">T&C</a></p>
                <p>Bank Offers 10% Unlimited Cashback On Flipkart ICICI Bank Credit Card <a href="#">T&C</a></p>
                <p className="emi"> EMI  Available With Credit Cards</p>
                </div>
                <div>
                    <h5>Highlights</h5>
                    <ol>
                        <li>Supported Apps: Netflix|Disney+Hotstar|Youtube</li>
                        <li>Operating System: Tizen</li>
                        <li>Resolution: HD Ready 1366 x 768 Pixels</li>
                        <li>Sound Output: 20 W</li>
                        <li>Refresh Rate: 60 Hz</li>
                        <li>Dual Nano SIM Slot and Memory Card Slot</li>
                        <li>VOOC Flash Charge 3.0</li>
                    </ol>
                </div>
                <div>
                    <button type="button" className="btn btn-warning brand">samsung</button>
                    <p>Brand Warranty of 1 Year Available For TV and 6 Months for Accessories</p>
                </div>
            </div>
            </div>
        </div>
        <Footer/>
        </div>
    )   
    }
};